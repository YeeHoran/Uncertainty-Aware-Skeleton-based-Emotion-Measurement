...
def parse_args():
    ...
    parser.add_argument('-freq', '--print_freq', default=576)
...
def validate(test_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for idx, (batch_input, batch_gt) in tqdm(enumerate(test_loader)):
            ...
            acc1, acc3 = accuracy(output, batch_gt, topk=(1, 3))
...
def train_with_config(args, opts):
    ...
    criterion_contra = SupConProtoLoss(
        num_classes=args.action_classes,  # 7类
        ...
        for epoch in range(st, args.epochs):
            ...
            for idx, (batch_input, batch_gt) in tqdm(enumerate(train_loader)):    # (N, 2, T, 16, 4)
                ...
                if torch.cuda.is_available():
                    ...
                    # ====== 提取关节置信度 ======
                    joint_conf = batch_input[..., -1]  # [N, 2, T, 16]
                    joint_conf = joint_conf.mean(dim=1)  # 若有两个模态，则在模态维求平均 → [N, T, 16]

                # Mixed precision context manager for forward pass
                with autocast():
                    # 前向
                    output = model(batch_input, joint_conf=joint_conf)  # feature: [N, D], output: [N, num_classes]; # ← 新增参数
                    #feature, output = model.module(batch_input, joint_conf=joint_conf)
                    # ----------------------
                    # 交叉熵损失
                    # ----------------------
            ...
            # 验证阶段
            test_loss, test_top1, test_top3 = validate(test_loader, model, criterion)
            timestamp = datetime.now().strftime("%H:%M:%S")
            print(f'[{timestamp}] Epoch: {epoch} | Test Loss: {test_loss:.4f}, Acc@1: {test_top1:.2f}%, Acc@3: {test_top3:.2f}%')

            # 记录TensorBoard
            train_writer.add_scalar('train_loss', losses_train.avg, epoch + 1)
            train_writer.add_scalar('train_top1', top1.avg, epoch + 1)
            train_writer.add_scalar('train_top3', top3.avg, epoch + 1)  # ✅ 新增
            train_writer.add_scalar('test_loss', test_loss, epoch + 1)
            train_writer.add_scalar('test_top1', test_top1, epoch + 1)
            train_writer.add_scalar('test_top3', test_top3, epoch + 1)  # ✅ 新增
            ...
            torch.save({
                ...
                'latest_acc_top1' : top1,
                'latest_acc_top3' : top3,
            }, chk_path)

            # Save best checkpoint.
            # Save best checkpoint.
            best_top1_chk_path = os.path.join(opts.checkpoint, 'best_epoch_top1.bin')
            if test_top1 > best_acc_top1:
                print('acc1:test_top1')
                best_acc_top1 = test_top1
                best_acc_top3 = test_top3  # ✅ 保存 Top-3
                ...
                torch.save({
                    ...
                    'best_acc_top3': best_acc_top3  # ✅ 新增 Top-3
                }, best_top1_chk_path)

            best_top3_chk_path = os.path.join(opts.checkpoint, 'best_epoch_top3.bin')
            if test_top3 > best_acc_top3:
                print('acc3:test_top3')
                best_acc_top1 = test_top1
                best_acc_top3 = test_top3  # ✅ 保存 Top-3
                print("save best checkpoint")
                torch.save({
    ...
    if opts.evaluate:
        ...
        print('Loss {loss:.4f} \t'
              'Acc@1 {top1:.3f} \t'
              'Acc@3 {top3:.3f}'.format(
            loss=test_loss, top1=test_top1, top3=test_top3))
