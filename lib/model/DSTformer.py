...
class Attention(nn.Module):
    ...
    def forward(self, x, seqlen=1, frame_conf=None):
        B, N, C = x.shape
        ...
        if self.mode == 'series':
            ...
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)
            x = self.forward_temporal(q, k, v, seqlen=seqlen, frame_conf=frame_conf)
        elif self.mode == 'parallel':
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)
            x_t = self.forward_temporal(q, k, v, seqlen=seqlen, frame_conf=frame_conf)
    ...
    def forward_temporal(self, q, k, v, seqlen=8, frame_conf=None):
        B, _, N, C = q.shape
        ...
        # === 将 frame_conf 融入 Q 和 K ===
        if frame_conf is not None:
            # frame_conf: [B, T] -> [B, 1, 1, T, 1]
            conf = frame_conf.unsqueeze(1).unsqueeze(2).unsqueeze(-1)
            qt = qt * conf.sqrt()  # sqrt 用于保持点积期望不变
            kt = kt * conf.sqrt()
        # === 将 frame_conf 融入 Q 和 K ===
...
class Block(nn.Module):
    ...
    def forward(self, x, seqlen=1, frame_conf=None):
        if self.st_mode=='stage_st':
            ...
            # --- Temporal (加上 frame_conf) ---
            x = x + self.drop_path(self.attn_t(self.norm1_t(x), seqlen, frame_conf=frame_conf))
            ...
            x_t = x + self.drop_path(self.attn_t(self.norm1_t(x), seqlen, frame_conf=frame_conf))
...
class DSTformer(nn.Module):
    def __init__(self, dim_in=3, dim_out=7, dim_feat=256, dim_rep=512,
    ...
    def forward(self, x, joint_conf=None, return_rep=False):
        #print("forward called with joint_conf:", joint_conf is not None)
        B, F, J, C = x.shape
        x = x.reshape(-1, J, C)
        BF = x.shape[0]
        ...
        # ✅ Step 3: 计算每帧平均置信度 (frame_conf)
        if joint_conf is not None:
            frame_conf = joint_conf.mean(dim=-1)  # [B, T]
        else:
            frame_conf = None
        ...
        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            # ✅ Step 4: 把 frame_conf 传给 block
            x_st = blk_st(x, F, frame_conf=frame_conf)
            x_ts = blk_ts(x, F, frame_conf=frame_conf)
            if self.att_fuse:
    ...
    def get_representation(self, x, joint_conf=None):
        return self.forward(x, joint_conf=joint_conf, return_rep=True)
