...
class ActionNet(nn.Module):
    ...
    def forward(self, x, joint_conf=None):
        '''
        ...
        if joint_conf is not None:
            # 如果 joint_conf 是 (N, T, J)，则补充 M 维度
            if joint_conf.dim() == 3:
                joint_conf = joint_conf.unsqueeze(1)  # (N, 1, T, J)
            joint_conf = joint_conf.reshape(N * M, T, J)  # 对应 reshape

        feat = self.backbone.get_representation(x, joint_conf=joint_conf)
